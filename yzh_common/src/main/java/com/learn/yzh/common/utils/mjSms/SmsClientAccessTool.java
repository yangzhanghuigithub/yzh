package com.learn.yzh.common.utils.mjSms;
...
public class SmsClientAccessTool {
	...
	public String doAccessHTTPPost(String sendUrl, String sendParam,
		...
		try {
			...
			System.out.println("短信重复发送ceshi"+new Date().getTime()+sendParam+" == ========= "+sendUrl);
			...
			System.out.println("短信重复发送ceshi结束"+new Date().getTime()+sendParam+" == ========= "+sendUrl);
	...
	public String doAccessHTTPGet(String sendUrl, String backEncodType) {
		...
		try {
			System.out.println("短信进入时间"+new Date().getTime() +" ======== "+sendUrl);
			if (backEncodType == null || backEncodType.equals("")) {
				backEncodType = "UTF-8";
			}
			...
			System.out.println("短信发送结束时间"+new Date().getTime() +" ======== "+sendUrl);
		} catch (IOException e) {
			receive.append("访问产生了异常-->").append(e.getMessage());
			e.printStackTrace();
		} finally {
