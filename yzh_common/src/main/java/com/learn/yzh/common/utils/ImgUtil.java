...
public class ImgUtil {
	 ...
	 private static String DEFAULT_PREVFIX = "thumb";
    ...
    public void thumbnailImage(File imgFile, int w, int h, String prevfix, boolean force){
        if(imgFile.exists()){
            try {
                ...
                // 将图片保存在原目录并加上前缀
                File thumbPath = new File(p.substring(0, p.lastIndexOf(File.separator)) + File.separator + prevfix + File.separator);
                if(!thumbPath.exists()){
                	thumbPath.mkdir();
                }
                ImageIO.write(bi, suffix, new File(p.substring(0, p.lastIndexOf(File.separator)) + File.separator + prevfix + File.separator + imgFile.getName()));
            } catch (IOException e) {
    ...
    public static void main(String[] args) {
        new ImgUtil().thumbnailImage("L:/tmp/liuyifei.jpeg", 100, 150);
    }
